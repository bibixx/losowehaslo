...
import { toggleAdvancedOptionsOpen } from "./utils/toggleAdvancedOptionsOpen";
...
const setupEventListeners = () => {
  ...
  const $addSpecialCharacter = document.querySelector("#add-special-character") as HTMLInputElement;
  const $randomizeCase = document.querySelector("#randomize-case") as HTMLInputElement;
  const $advancedOptionsCheckbox = document.querySelector("#advanced-options-checkbox") as HTMLInputElement;
  ...
  $advancedOptionsCheckbox.addEventListener("input", () => {
    toggleAdvancedOptionsOpen($advancedOptionsCheckbox.checked);
  });
};

toggleAdvancedOptionsOpen(false);
renderAndGeneratePassword(currentSettings, currentSeed);
